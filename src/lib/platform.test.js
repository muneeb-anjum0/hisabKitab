...
describe('application contracts', () => {
  ...
  it('keeps Capacitor identity and output aligned with Android', () => {
    expect(capacitor).toMatchObject({
      appId: 'com.muneebanjum.hisabkitab',
      appName: 'HisabKitab',
      webDir: 'dist',
    });
  ...
  it('keeps native authentication and system-bar safety enabled', () => {
    expect(capacitor.plugins.FirebaseAuthentication).toMatchObject({
      skipNativeAuth: true,
  ...
  it('registers native back navigation and both Android quick-action widgets', () => {
    ...
    [
      'android/app/src/main/res/xml/widget_square_info.xml',
      'android/app/src/main/res/xml/widget_wide_info.xml',
    ].forEach((path) => {
  ...
  it('keeps Firebase Hosting configured as a single-page application', () => {
    ...
    expect(firebase.firestore).toEqual({
  ...
  it('documents every required public Firebase environment key', () => {
    const keys = [...read('.env.example').matchAll(/^VITE_FIREBASE_[A-Z_]+(?==)/gm)].map(
      ([key]) => key,
    );
    expect(keys).toEqual([
  ...
  it('keeps Firebase policy files and the stable APK present', () => {
    ['firestore.rules', 'firestore.indexes.json', 'HisabKitab.apk'].forEach((path) =>
      expect(existsSync(new URL(`../../${path}`, import.meta.url))).toBe(true),
    );
  });
});
