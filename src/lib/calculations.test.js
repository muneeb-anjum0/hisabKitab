...
describe('financial ledger', () => {
  ...
  it('calculates monthly income and expenses', () => {
    const transactions = [
      { type: 'expense', amount: 1000, date: '2026-09-04' },
      { type: 'expense', amount: 500, date: '2026-08-31' },
    ];
    expect(monthlyTotals('2026-09', transactions, remittances)).toEqual({
      received: 30000,
      spent: 1000,
      remaining: 29000,
    });
  ...
  it('keeps multiple years of months ordered without mixing their ledgers', () => {
    const transactions = [
      { type: 'expense', amount: 10, date: '2025-12-31' },
      { type: 'expense', amount: 20, date: '2026-01-01' },
      { type: 'expense', amount: 30, date: '2026-09-09' },
    ];
    const income = [
      { totalAmount: 100, receivedAt: '2025-12-01' },
      { totalAmount: 200, receivedAt: '2026-01-02' },
    ];
    expect(ledgerMonths(transactions, income, '2026-10')).toEqual([
      '2026-10',
      '2026-09',
      '2026-01',
      '2025-12',
    ]);
    expect(monthlyTotals('2025-12', transactions, income)).toEqual({
      received: 100,
      spent: 10,
      remaining: 90,
    });
    expect(monthlyTotals('2026-01', transactions, income)).toEqual({
      received: 200,
      spent: 20,
      remaining: 180,
    });
  ...
  it('preserves the requested end-to-end money journey', () => {
    ...
    transactions = transactions.map((item) =>
      item.id === 'groceries' ? { ...item, amount: 1000 } : item,
    );
  ...
  it('uses FIFO across multiple lots for the Baba Kitchen scenario', () => {
    ...
    const earlyExpenses = [2500, 1200, 3000].map((amount, index) => ({
      id: `e${index}`,
      fundId: 'kitchen',
      type: 'expense',
      amount,
    ...
    let lots = buildMoneyLots(
      'kitchen',
      kitchenAllocations.slice(0, 1),
      kitchenRemittances,
      earlyExpenses,
    );
    ...
    expect(usage).toEqual({
      usages: [
        { lotId: 'lot-1', amount: 8300 },
        { lotId: 'lot-2', amount: 1700 },
      ],
      uncovered: 0,
    });
    const finalLots = buildMoneyLots('kitchen', kitchenAllocations, kitchenRemittances, [
      ...earlyExpenses,
      {
        id: 'e4',
        fundId: 'kitchen',
        ...
        amount: 10000,
        date: '2026-09-11',
        lotUsages: usage.usages,
      },
    ]);
    expect(finalLots.map((lot) => lot.remaining)).toEqual([0, 13300]);
    expect(
      fundTotals('kitchen', kitchenAllocations, [
        ...earlyExpenses,
        { fundId: 'kitchen', type: 'expense', amount: 10000 },
      ]).remaining,
    ).toBe(13300);
  });
  ...
  it('honors a manually selected lot then continues FIFO for the remainder', () => {
    const lots = [
      { id: 'old', remaining: 3000 },
      { id: 'chosen', remaining: 2000 },
      { id: 'next', remaining: 4000 },
    ];
    expect(consumeMoneyLots(lots, 4500, 'chosen')).toEqual({
      usages: [
        { lotId: 'chosen', amount: 2000 },
        { lotId: 'old', amount: 2500 },
      ],
      uncovered: 0,
    });
  });
  ...
  it('creates a derived destination lot for a transfer', () => {
    const transfer = {
      ...
      linkId: 'x',
      fundId: 'house',
      sourceFundName: 'Personal',
      type: 'transfer',
      amount: 2000,
      date: '2026-09-12',
    };
    expect(buildMoneyLots('house', [], [], [transfer])[0]).toMatchObject({
      id: 'transfer:transfer-in',
      kind: 'transfer',
      source: 'Transfer from Personal',
      originalAmount: 2000,
    });
  });

  it('describes normal, zero-allocation, and overspent Fund cards accurately', () => {
    expect(fundCardState({ allocated: 15000, remaining: 10000 })).toEqual({
      kind: 'normal',
      label: '67% LEFT',
      percentage: 67,
    });
    expect(fundCardState({ allocated: 0, remaining: 0 })).toEqual({
      kind: 'empty',
      label: 'NO MONEY YET',
      percentage: null,
    });
    expect(fundCardState({ allocated: 0, remaining: -10340 })).toEqual({
      kind: 'overspent',
      label: 'OVERSPENT',
      overBy: 10340,
      percentage: null,
  ...
  it('excludes archived Funds from active portfolio totals', () => {
    const result = portfolioTotals(
      [{ id: 'active' }, { id: 'old', archived: true }],
      [
        { fundId: 'active', amount: 10 },
        { fundId: 'old', amount: 50 },
      ],
      [],
      [],
    );
    expect(result.funds.map((fund) => fund.id)).toEqual(['active']);
  ...
  it('allows deletion only for a completely empty Fund', () => {
    const base = {
      allocations: [],
      transactions: [],
      memberships: [{ fundId: 'empty', role: 'owner' }],
    };
    expect(fundDeletionAssessment('empty', base).empty).toBe(true);
    expect(
      fundDeletionAssessment('empty', { ...base, allocations: [{ fundId: 'empty', amount: 1 }] })
        .empty,
    ).toBe(false);
    expect(
      fundDeletionAssessment('empty', {
        ...base,
        transactions: [{ fundId: 'empty', type: 'expense', amount: 1 }],
      }).empty,
    ).toBe(false);
    expect(
      fundDeletionAssessment('empty', {
        ...base,
        memberships: [...base.memberships, { fundId: 'empty', role: 'viewer' }],
      }).empty,
    ).toBe(false);
  });
  ...
  it('updates only the edited Fund in local state', () => {
    const original = [
      { id: 'house', name: 'House', accent: 'red' },
      { id: 'gym', name: 'Gym', accent: 'green' },
    ];
  ...
  it('sorts and rearranges Fund cards without losing ids', () => {
    ...
    expect(
      sortFunds([
        { id: 'z', createdAt: '2026-01-02' },
        { id: 'a', createdAt: '2026-01-01' },
      ]).map((fund) => fund.id),
    ).toEqual(['a', 'z']);
    expect(moveFund(['first', 'middle', 'last'], 'middle', -1)).toEqual([
      'middle',
      'first',
      'last',
    ]);
    expect(moveFund(['first', 'middle', 'last'], 'first', -1)).toEqual(['first', 'middle', 'last']);
    expect(placeFund(['first', 'middle', 'last'], 'last', 'first')).toEqual([
      'last',
      'first',
      'middle',
    ]);
    expect(placeFund(['first', 'middle', 'last'], 'first', 'last')).toEqual([
      'middle',
      'last',
      'first',
    ]);
    expect(placeFund(['first', 'middle', 'last'], 'first', 'middle')).toEqual([
      'middle',
      'first',
      'last',
    ]);
...
describe('calculation edge cases', () => {
  it('sums numeric strings and safely ignores invalid values', () => {
    expect(
      sum(
        [{ value: '12.5' }, { value: null }, { value: 'nope' }, { value: -2.5 }],
        (item) => item.value,
      ),
    ).toBe(10);
  });
  ...
  it('normalizes Firestore, seconds, ISO, and missing timestamps', () => {
    ...
    expect(timestampMillis('2026-09-01T00:00:00.000Z')).toBe(
  ...
  it('keeps unrelated Funds and transaction types out of Fund totals', () => {
    const result = fundTotals(
      'home',
      [
        { fundId: 'home', amount: '100' },
        { fundId: 'other', amount: 900 },
      ],
      [
  ...
  it('reports transferred-in state when remaining exceeds the available denominator', () => {
    expect(fundCardState({ allocated: 100, remaining: 140 })).toEqual({
      kind: 'inflow',
      label: 'TRANSFERRED IN',
      percentage: null,
    });
    expect(fundCardState({ allocated: 0, adjustments: 50, remaining: 50 })).toEqual({
      kind: 'normal',
      label: '100% LEFT',
      percentage: 100,
    });
  ...
  it('counts counterparty transfers as Fund history that blocks deletion', () => {
    const data = {
      ...
      transactions: [
        { fundId: 'other', counterpartyFundId: 'home', type: 'transfer', amount: -40 },
      ],
    };
    expect(fundDeletionAssessment('home', data)).toMatchObject({
      empty: false,
      transactionCount: 1,
      transferCount: 1,
    });
  ...
  it('excludes transfers and out-of-month records from monthly spending', () => {
    expect(
      monthlyTotals(
        ...
        [
          { totalAmount: 50, receivedAt: '2026-09-01' },
          { totalAmount: 70, receivedAt: '2026-08-31' },
        ],
      ),
    ).toEqual({ received: 50, spent: 10, remaining: 40 });
  });

  it('orders Money Lots by received date, creation time, then id', () => {
    const lots = buildMoneyLots(
      'home',
      [
    ...
    expect(lots.map((lot) => [lot.id, lot.number])).toEqual([
      ['a', 1],
      ['b', 2],
    ]);
  });
  ...
  it('falls back to FIFO when a saved usage references a missing lot', () => {
    const lots = buildMoneyLots(
      'home',
      [{ id: 'real', fundId: 'home', amount: 100 }],
      [],
      [
        {
          id: 'expense',
          ...
          amount: 35,
          date: '2026-09-01',
          lotUsages: [{ lotId: 'missing', amount: 35 }],
        },
      ],
    );
  ...
  it('reports uncovered spending when available Money Lots are insufficient', () => {
    ...
    ).toEqual({ usages: [{ lotId: 'one', amount: 20 }], uncovered: 15 });
  });

  it('treats invalid and negative requested consumption as zero', () => {
    expect(consumeMoneyLots([{ id: 'one', remaining: 20 }], -5)).toEqual({
      usages: [],
      uncovered: 0,
    });
    expect(consumeMoneyLots([{ id: 'one', remaining: 20 }], 'invalid')).toEqual({
      usages: [],
      uncovered: 0,
    });
  });
