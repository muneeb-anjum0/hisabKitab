...
export default function FundDetail() {
  ...
  if (!fund)
    return <Empty title="FUND NOT FOUND.">This Fund may have been archived or removed.</Empty>;
  ...
  const transactions = data.transactions
    ...
    .sort((a, b) => {
      ...
      const created = (value) =>
        typeof value?.toMillis === 'function' ? value.toMillis() : Date.parse(value || 0) || 0;
      return (
        created(b.createdAt) - created(a.createdAt) || String(b.id).localeCompare(String(a.id))
      );
    });
  ...
  return (
    <>
      ...
      <section className={`fund-detail-hero ${fund.accent}`}>
        <div>
          <span>{fund.type === 'shared' ? 'SHARED FUND' : 'PERSONAL FUND'}</span>
          <h1>{fund.name}</h1>
          <p>
            {members.length} member{members.length !== 1 ? 's' : ''} · {lotSummary.lots.length}{' '}
            money lot{lotSummary.lots.length !== 1 ? 's' : ''}
          </p>
        </div>
        <div>
          ...
          <div className="fund-hero-stats">
            <span>
              RECEIVED <b>{money(totals.allocated + Math.max(0, totals.adjustments))}</b>
            </span>
            <span>
              SPENT <b>{money(totals.spent)}</b>
            </span>
          </div>
          <Progress
            value={totals.spent}
            max={totals.allocated}
            label={`${money(totals.spent)} spent`}
          />
        </div>
      </section>
      ...
      <div className="section-heading lot-heading">
        <div>
          <span className="kicker">BATCH BY BATCH</span>
          <h2>MONEY LOTS</h2>
        </div>
      </div>
      <section className="money-lots">
        {lotSummary.lots.length ? (
          [...lotSummary.lots].reverse().map((lot) => (
            <article className="money-lot" key={lot.id}>
              <header>
                <span>MONEY LOT</span>
                <b>#{String(lot.number).padStart(2, '0')}</b>
              </header>
              <small>
                {lot.kind === 'transfer' ? 'TRANSFER LOT' : `FROM ${lot.source.toUpperCase()}`}
              </small>
              <strong>{money(lot.originalAmount)}</strong>
              <div>
                <span>
                  RECEIVED <b>{friendlyDate(lot.receivedAt)}</b>
                </span>
                <span>
                  SPENT <b>{money(lot.spent)}</b>
                </span>
                <span>
                  LEFT <b>{money(lot.remaining)}</b>
                </span>
              </div>
              <Progress
                value={lot.spent}
                max={lot.originalAmount}
                label={`${Math.round((lot.remaining / lot.originalAmount) * 100) || 0}% left`}
              />
            </article>
          ))
        ) : (
          <Empty title="NO MONEY LOTS YET.">
            When money arrives for this Fund, each allocation will land here.
          </Empty>
        )}
      </section>
...
function Members({ fund, members, data, isOwner, onClose }) {
  ...
  return (
    <Modal title="FUND MEMBERS" onClose={onClose}>
      <div className="member-list">
        {members.map((member) => (
          <div key={member.id}>
            <span className="avatar">
              {(member.displayName || member.email || 'Y')[0].toUpperCase()}
            </span>
            <p>
              <b>{member.displayName || member.email || 'You'}</b>
              <small>{member.email || 'Fund owner'}</small>
            </p>
            {isOwner && member.role !== 'owner' ? (
              <>
                <ComicSelect
                  compact
                  label={`ROLE FOR ${member.displayName || member.email}`}
                  value={member.role}
                  onChange={(nextRole) => quietly(data.updateMember(member.id, nextRole))}
                  options={[
                    ['editor', 'Editor'],
                    ['viewer', 'Viewer'],
                  ]}
                ...
                <button
                  ...
                  onClick={() => quietly(data.removeMember(member.id))}
                >
                  ×
                </button>
              </>
      ...
      {isOwner && (
        <form onSubmit={add}>
          ...
          <ComicSelect
            ...
            onChange={setRole}
            options={[
              ['editor', 'Editor — can spend'],
              ['viewer', 'Viewer — read only'],
            ]}
          />
          {error && <p className="form-error">{error}</p>}
          <Button disabled={busy || !email}>{busy ? 'ADDING…' : 'ADD TO FUND'}</Button>
        </form>
      )}
    </Modal>
  );
