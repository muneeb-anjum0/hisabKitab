...
export default function Funds() {
      ...
      >
        ...
        <Link className="fund-strip-link" to={`/funds/${fund.id}`}>
          ...
          <div className="fund-strip-numbers">
            <span>
              ALLOCATED <b>{money(totals.allocated)}</b>
            </span>
            <span>
  ...
  return (
    <>
      <div className="page-title funds-title">
        <span className="kicker">EVERY RUPEE HAS A JOB</span>
        <h1>THE FUNDS</h1>
        <p>Separate pockets. One honest ledger.</p>
        <Button onClick={() => setCreating(true)}>+ NEW FUND</Button>
      </div>
      <section className="fund-list">
        {activeFunds.map(renderFund)}
        {!activeFunds.length && (
          <Empty
            title="NO ACTIVE FUNDS."
            action={<Button onClick={() => setCreating(true)}>CREATE A FUND</Button>}
          >
            Create a Fund or restore one from the archive.
          </Empty>
        )}
      </section>
      {archivedFunds.length > 0 && (
        <section className="archived-funds">
          <button
            className="archive-toggle"
            aria-expanded={showArchived}
            onClick={() => setShowArchived(!showArchived)}
          >
            {showArchived ? '− HIDE ARCHIVED' : `+ SHOW ARCHIVED (${archivedFunds.length})`}
          </button>
          {showArchived && <div className="fund-list">{archivedFunds.map(renderFund)}</div>}
        </section>
      )}
      {creating && (
        <NewFund
...
function NewFund({ onClose, onSave }) {
  ...
  return (
    <Modal title="CREATE FUND" onClose={onClose}>
      <form onSubmit={submit}>
        ...
        <Button disabled={busy || !values.name.trim()}>{busy ? 'CREATING…' : 'CREATE FUND'}</Button>
      </form>
    </Modal>
  );
}
