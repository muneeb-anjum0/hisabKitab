...
function IncomeRow({ item, onEdit, onDelete }) { return <article className="income-row"><div><small>MONEY RECEIVED · {friendlyDate(item.activityDate)}</small><strong>{item.description}</strong>{item.note && <span>{item.note}</span>}</div><div className="income-value-actions"><b>+{money(item.amount)}</b><div className="income-actions"><button className="income-edit" onClick={onEdit} aria-label={`Edit ${item.description}`} title="Edit">✎</button><button className="income-delete" onClick={onDelete} aria-label={`Delete ${item.description}`} title="Delete">×</button></div></div></article>; }
