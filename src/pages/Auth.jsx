  ...
  return (
    <main className="setup-page">
      ...
      <section className="setup-card panel">
        ...
        <p>
          Firebase credentials have not been added yet. No account or financial data can be used
          until the app is connected.
        </p>
        <code>.env.local</code>
        <ul>
          {required.map((item) => (
        ...
        <p>
          Copy the six values from your Firebase Web App configuration, save the file, then restart:
        </p>
        <pre>npm run dev</pre>
...
export default function Auth() {
  ...
  const submit = async (event) => {
    ...
    try {
      if (isSignup)
        await auth.emailSignup(values.name.trim(), values.email.trim(), values.password);
  ...
  return (
    <main className="auth-page">
      ...
      <section className="auth-form panel">
        ...
        <p>
          {isSignup ? 'Start a ledger that actually makes sense.' : 'Your money story is waiting.'}
        </p>
        <form onSubmit={submit}>
          ...
          <button type="button" className="google" disabled={busy} onClick={google}>
            G&nbsp; CONTINUE WITH GOOGLE
          </button>
        </form>
        {!isSignup && (
          <button className="forgot" disabled={busy} onClick={reset}>
            Forgot password?
          </button>
        )}
        <button
