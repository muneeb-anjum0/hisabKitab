...
export default function Dashboard({ onAction }) {
  ...
  const [allocating, setAllocating] = useState(false); const [search, setSearch] = useState(''); const [rearranging, setRearranging] = useState(false); const [activeId, setActiveId] = useState(null); const [orderedFunds, setOrderedFunds] = useState([]); const [pressedAction, setPressedAction] = useState('');
  ...
  const runAction = (action) => {
    if (!window.matchMedia('(max-width: 800px)').matches) { onAction(action); return; }
    setPressedAction(action); window.setTimeout(() => { setPressedAction(''); onAction(action); }, 140);
  };
  ...
  return <div className="dashboard-page">
    ...
    <div className="dashboard-grid">
      ...
      <aside className="dashboard-aside"><div className="sticky-note"><b>IT ALL<br/>ADDS UP.</b></div><div className="direct-actions"><button className={`primary ${pressedAction === 'remittance' ? 'is-pressed' : ''}`} onClick={() => runAction('remittance')}>＋ ADD MONEY</button><button className={pressedAction === 'expense' ? 'is-pressed' : ''} onClick={() => runAction('expense')}>＋ ADD EXPENSE</button><button className={pressedAction === 'transfer' ? 'is-pressed' : ''} onClick={() => runAction('transfer')}>⇄ TRANSFER MONEY</button><button className={pressedAction === 'fund' ? 'is-pressed' : ''} onClick={() => runAction('fund')}>＋ CREATE FUND</button></div><Reminder totals={totals} funds={activeFunds} data={data}/><JourneyPanel/><div className="lot-watch"><small>MONEY LOT WATCH</small><b>{data.allocations.length} LOT{data.allocations.length !== 1 ? 'S' : ''} ON THE BOOKS</b><span>Oldest money is spent first unless you choose a specific lot.</span></div></aside>
    </div>
