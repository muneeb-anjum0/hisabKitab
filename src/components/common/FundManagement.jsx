...
export function EditFund({ fund, onClose, onSave }) {
  ...
  return <Modal title="EDIT FUND" onClose={onClose}><form onSubmit={async (event) => { event.preventDefault(); if (!values.name.trim() || busy) return; setBusy(true); try { await onSave({ ...values, name: values.name.trim() }); } catch (saveError) { setError(saveError.message); setBusy(false); } }}><Field label="FUND NAME"><input data-autofocus required maxLength="35" value={values.name} onChange={(event) => setValues({ ...values, name: event.target.value })}/></Field><ComicSelect label="TYPE" value={values.type} onChange={(type) => setValues({ ...values, type })} options={[["personal", "Personal"], ["shared", "Shared"]]}/><fieldset className="swatches"><legend>ACCENT</legend>{accents.map((accent) => <button type="button" aria-label={`${accent} accent`} className={`${accent} ${values.accent === accent ? 'active' : ''}`} onClick={() => setValues({ ...values, accent })} key={accent}/>)}</fieldset>{error && <p className="form-error">{error}</p>}<Button disabled={busy || !values.name.trim()}>{busy ? 'SAVING…' : 'SAVE FUND'}</Button></form></Modal>;
}
