...
export function Button({ children, variant = 'ink', className = '', ...props }) {
  return (
    <button className={`ink-button ${variant} ${className}`} {...props}>
      {children}
    </button>
  );
}
...
export function ComicSelect({
  ...
  const selected =
    options.find(([id]) => id === value)?.[1] || options[0]?.[1] || 'Nothing to pick';
  ...
  const popover =
    ...
    createPortal(
      <div
        className="comic-options-layer"
        onPointerDown={(event) => event.target === event.currentTarget && setOpen(false)}
      >
        <div className="comic-options" ref={menu} role="listbox" aria-label={label}>
          {options.map(([id, text], index) => (
            <button
              ...
              className={picked === id ? 'comic-option-picked' : ''}
              style={{ '--option-index': index }}
              key={id}
              onClick={() => pick(id)}
            >
              {text}
              <b>{picked === id ? 'POW!' : id === value ? '✓' : ''}</b>
            </button>
          ))}
        </div>
      </div>,
  ...
  return (
    <>
      <div className={`comic-select ${open ? 'open' : ''} ${compact ? 'compact' : ''}`} ref={root}>
        <span>{label}</span>
        <button
          type="button"
...
export function ComicDatePicker({ label, value, onChange, mode = 'date', allowClear = false }) {
  ...
  const days =
    mode === 'date'
      ? Array.from(
          { length: new Date(view.year, view.month + 1, 0).getDate() },
          (_, index) => index + 1,
        )
      : [];
  const blanks =
    mode === 'date'
      ? Array.from({ length: (new Date(view.year, view.month, 1).getDay() + 6) % 7 })
      : [];
  const display = value
    ? mode === 'month'
      ? `${monthNames[Number(value.slice(5, 7)) - 1]} ${value.slice(0, 4)}`
      : new Date(`${value}T12:00:00`).toLocaleDateString('en-PK', {
          day: '2-digit',
          month: 'short',
          year: 'numeric',
        })
  ...
  const calendar =
    ...
    createPortal(
      <div
        ...
        onPointerDown={(event) => event.target === event.currentTarget && setOpen(false)}
      >
        <section className="comic-calendar" role="dialog" aria-modal="true" aria-label={label}>
          <header>
            <button
              type="button"
              onClick={() =>
                mode === 'month' ? setView({ ...view, year: view.year - 1 }) : shift(-1)
              }
              aria-label="Previous"
            >
              ←
            </button>
            <b>{mode === 'month' ? view.year : `${monthNames[view.month]} ${view.year}`}</b>
            <button
              type="button"
              onClick={() =>
                mode === 'month' ? setView({ ...view, year: view.year + 1 }) : shift(1)
          ...
          {mode === 'month' ? (
            <div className="comic-months">
              {monthNames.map((month, index) => (
                <button
                  type="button"
                  className={value === `${view.year}-${padDate(index + 1)}` ? 'selected' : ''}
              ...
              <div className="comic-days">
                ...
                {days.map((day) => (
                  <button
                    ...
                    className={
                      day === selectedDay &&
                      value?.startsWith(`${view.year}-${padDate(view.month + 1)}`)
                        ? 'selected'
                        : ''
                    }
  ...
  return (
    <>
      <div className="comic-select comic-date">
        <span>{label}</span>
        <button
          type="button"
          onClick={() => {
            ...
            setView({ year: Number(source.slice(0, 4)), month: Number(source.slice(5, 7)) - 1 });
            setOpen(true);
          }}
...
export function Modal({ title, onClose, children, wide = false }) {
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => {
      if (!dialog.querySelector('[data-autofocus]'))
