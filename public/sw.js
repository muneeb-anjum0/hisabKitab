...
self.addEventListener('activate', (event) => event.waitUntil(
  caches.keys().then((keys) => Promise.all(
    keys.filter((key) => key.startsWith('hisabkitab-') && !OWNED_CACHES.includes(key)).map((key) => caches.delete(key)),
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.hostname === 'fonts.googleapis.com' || url.hostname === 'fonts.gstatic.com') {
    event.respondWith(caches.match(event.request).then((cached) => cached || fetch(event.request)
      ...
      .then((response) => { void trimCache(FONT_CACHE, 20); return response; })));
    return;
  }
  ...
  if (immutableAsset) {
    event.respondWith(cached.then((response) => response || fetch(event.request)
      .then((fresh) => cacheResponse(ASSET_CACHE, event.request, fresh))));
    return;
  }
  const fresh = fetch(event.request).then((response) => cacheResponse(ASSET_CACHE, event.request, response))
    .then((response) => { void trimCache(ASSET_CACHE, 80); return response; })
    .catch(() => cached);
  event.waitUntil(fresh.then(() => undefined));
